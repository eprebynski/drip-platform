...
| R25 | Private evidence automation could be misused as proof that raw exports are clean or migration-ready. | Medium | Medium | Scanner false negatives or incomplete templates could allow sensitive data or unresolved dependencies into planning docs. | Treat the kit as a local helper only; review redaction reports manually, keep `UNKNOWN` defaults until verified, and require Drip/ChatGPT approval before migration decisions. | Security/Operations |
| R26 | Private evidence inbox auto-classification may route a file to the wrong category or hide an ambiguous dependency. | Medium | Medium | Migration planning could miss a live dependency if classification output is trusted without review. | Send low/unknown confidence files to `review-needed/`, preserve import manifests, run status reports, scan before use, and require human review before sanitized summaries or Phase 3 decisions. | Security/Operations |
...
| Private evidence automation kit | Can create local folders/templates and scan text files, but cannot prove complete redaction or production readiness. | Use only outside the repo; review reports manually; no live credentials, private API calls, production changes, or Phase 3 work. |
| Private evidence inbox importer | Can classify and copy private files locally, but cannot prove a category is correct or safe to summarize. | Treat manifests as review aids; inspect `review-needed/`; run the scanner; keep `UNKNOWN` until verified. |
...
The aggregate and repo ZIP resolved many earlier planning unknowns. Phase 2.2 reduces the Squarespace evidence gap with a public read-only inventory, Phase 2.3 defines the private-source export plan, Phase 2.4 adds local automation for folder creation, templates, manifests, and redaction reports, and Phase 2.5 adds a local inbox importer/status workflow. Private exports are still required before migration decisions can be finalized. Remaining unmeasured risks are direct deployed Apps Script source parity, Apps Script runtime source order, live traffic volume by route, live trigger health beyond the aggregate snapshot, current secret values/rotation status, live BigQuery table location/row freshness, Cloud Run IAM/deployed image parity, full branch/uncommitted repo state, Squarespace form destinations/notifications, Squarespace redirect settings, Squarespace commerce configuration/orders, upload service backend/storage, authoritative DNS TTLs, registrar account ownership, analytics/Search Console ownership, current DNS record dependencies not visible from public lookups, whether raw private exports can be safely redacted into commit-worthy summaries, whether all scanner findings are manually reviewed, and whether inbox auto-classification is reviewed before summaries are trusted.
